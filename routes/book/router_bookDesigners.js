module.exports = app => {
    ...
    const DiagrammingsBook = require('../../model').diagramadores_livros
    ...
    passport.authenticate('jwt', {
        ...
        try {
            ...
            if (successAuth) {
                const diagrammings_book_post = await DiagrammingsBook.create({
                    id: req.body.id,
                    diagramador_id: req.body.diagramador_id, 
                    livro_id: req.body.livro_id,
                })
                res.status(200).send(JSON.stringify({
                    "full_data": diagrammings_book_post,
                    "identificador_diagramadores_livro": diagrammings_book_post.id,
                    "status": Boolean(true),
                }));
            }  else {  
    ...
    passport.authenticate('jwt', {
        ...
        try {
            ...
            if (successAuth) {
                const diagrammings_book_put = await DiagrammingsBook.update({
                    diagramador_id: req.body.diagramador_id, 
                    livro_id: req.body.livro_id,
                },{
                    where: {
                ...
                res.status(200).send(JSON.stringify({
                    "full_data": diagrammings_book_put,
                    "status": Boolean(true),
                }));
            };
        } catch (error) {          
    ...
    passport.authenticate('jwt', {
        ...
        try {
            ...
            if (successAuth) {
                const diagrammings_book_get = await DiagrammingsBook.findAll({
                order: [
                    ['id', 'ASC']
                ]
            }); 
            res.status(200).send(diagrammings_book_get);  
            } else {
                console.error('Error: ',errorAuth)
                res.status(401).send(JSON.stringify({
                    "error": errorAuth,
