module.exports = app => {
    ...
    const RespCoversBook = require('../../model').resp_capas_livros
    ...
    passport.authenticate('jwt', {
        ...
        try {
            ...
            if (successAuth) {
                const resp_covers_book_post = await RespCoversBook.create({
                    id: req.body.id,
                    capas_id: req.body.capas_id,
                    livro_id: req.body.livro_id,
                })
                res.status(200).send(JSON.stringify({
                    "full_data": resp_covers_book_post,
                    "identificador_resp_cap_livro": resp_covers_book_post.id,
                    "status": Boolean(true),
                }));
            }  else {  
    ...
    passport.authenticate('jwt', {
        ...
        try {
            ...
            if (successAuth) {
                const resp_covers_book_put = await RespCoversBook.update({                    
                    capas_id: req.body.capas_id,
                    livro_id: req.body.livro_id,
                },{
                    where: {
                ...
                res.status(200).send(JSON.stringify({
                    "full_data": resp_covers_book_put,
                    "status": Boolean(true),
                }));
            };
        } catch (error) {          
    ...
    passport.authenticate('jwt', {
        ...
        try {
            ...
            if (successAuth) {
                const capas_get = await RespCoversBook.findAll({
                order: [
                    ['id', 'ASC']
                ]
            }); 
